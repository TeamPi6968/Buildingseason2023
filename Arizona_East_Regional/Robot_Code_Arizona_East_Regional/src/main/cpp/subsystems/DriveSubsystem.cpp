...
void DriveSubsystem::Drive(units::meters_per_second_t xSpeed,
  ...
  auto states = kDriveKinematics.ToSwerveModuleStates(
      fieldRelative ? frc::ChassisSpeeds::FromFieldRelativeSpeeds(
                          xSpeed, ySpeed, rot,
                          frc::Rotation2d(units::radian_t{m_gyro.GetYaw()}))
...
std::pair<double, double> DriveSubsystem::FOV_joystick(double X, double Y,
                                                       double heading,
                                                       double angle) {
  double forward;
  double strafe;
  ...
  strafe = -Y * sin(error_rad) + X * cos(error_rad);
  forward = Y * cos(error_rad) + X * sin(error_rad);

  return {strafe, forward};
}
double DriveSubsystem::GetAngle() { return m_gyro.GetYaw(); }
