...
class Outtake_Test {
 private:
  ...
  frc::Debouncer bouncer{100_ms, frc::Debouncer::kFalling};
  bool pressed = false;
  double offset = 0;
...
#pragma region Outtake Class Variables
  ...
  Motor armMotor{
    ...
    // Gripper_Motor
  Motor gripperMotor{
      Gripper_Motor_Can_Id,       Gripper_Motor_Type,         Gripper_Motor_Speed_Max,
...
#pragma endregion
 ...
 public:
  ...
  void run();
  void autonomous();
  void run_motors_auto(double Speed_Arm_Motor);
  void stop_motors_auto();
  void runToPositionAuto(int target_Angle);
  void run_Gripper_Motor_Auto();
  void stop_Gripper_Motor_Auto();
};
