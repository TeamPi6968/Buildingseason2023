...
std::pair<double, double> Outtake_Test::positionSelection(
  ...
  if (toggle_positions[Indexer_Position]) {
    ...
    start_moving = true;
    Gripper_Motor_Speed_Max_Out = Gripper_Motor_Speed_Zero;
...
void Outtake_Test::addLiftAngle() {
  bool up_debounce = bouncer.Calculate(joystick->GetPOV() == 0);
  bool down_debounce = bouncer.Calculate(joystick->GetPOV() == 180);
  if (up_debounce && !pressed) {
    offset += 5;
    pressed = true;
  }
  else if (!up_debounce && pressed){
    pressed = false;
  }
  if (down_debounce && !pressed) {
    offset -= 5;
    pressed = true;
  } else if (!down_debounce && pressed) {
    pressed = false;
  }
}
...
void Outtake_Test::runToPosition() {
  ...
  liftLeftMotor.SetSpeed(
      liftPID->Calculate(current_angle, target_positions.first + offset));
  frc::SmartDashboard::PutNumber("targetpos_offset", target_positions.first + offset);
...
//------------------------------------------Start Auto Arm 24-3-2023
void Outtake_Test::run_motors_auto(double Speed_Arm_Motor) {
    armMotor.SetSpeed(Speed_Arm_Motor);
  }

  void Outtake_Test::stop_motors_auto() {
    static double current_position_auto = getArmPosition();
    armMotor.SetSpeed(armPID->Calculate(getArmPosition(), current_position_auto));
  }

void Outtake_Test::runToPositionAuto(int target_Angle){
  static std::pair<double, double> target_positions{target_Angle, Mid_Length}; //{Home_Angle, Home_Length};
  static double current_angle = Home_Angle;
  static double current_position = Home_Length;
  //static bool position_state[3] = {true};
  static bool start_moving = false;
  current_angle = getLiftPosition();
 ...
 //     positionSelection(position_state, start_moving);  // mandatory
  ...
  liftLeftMotor.SetSpeed(
      liftPID->Calculate(current_angle, target_Angle));
  frc::SmartDashboard::PutNumber("targetpos_offset", target_positions.first);
  liftRightMotor.GetSpark().Follow(liftLeftMotor.GetSpark(), true);
}

//Naar homing schrijven

void Outtake_Test::run_Gripper_Motor_Auto(){
  gripperMotor.SetSpeed(-0.2);
}

void Outtake_Test::stop_Gripper_Motor_Auto(){
  gripperMotor.SetSpeed(0);
}

//-------------------------------------------- Stop Auto Arm 24-3-2023
...
void Outtake_Test::gripperMotorActuation(){
  
if (joystick->GetRawButton(5)){ //5 is L1
    gripperMotor.SetSpeed(1);//Gripper_Motor_Speed_In);
  }
else if (joystick->GetRawButton(6)){ //6 is R1
  gripperMotor.SetSpeed(-1);//-Gripper_Motor_Speed_Max_Out);
  }
//else if (joystick->GetPSButton()){
//gripperMotor.SetSpeed(-Gripper_Motor_Speed_Out_ChargeStation_DreamJoep);
//}
else{
  gripperMotor.SetSpeed(0);
}
